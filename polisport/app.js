...
// 6. FORGOT PASSWORD WIZARD CONTROLLERS
function openForgotWizard(event) {
    if (event) event.preventDefault();
    
    // Hide login validation errors if any
    document.getElementById('login-error-msg').style.display = 'none';

    // Show verification form, hide reset form, clear inputs
...
async function handleForgotVerify(event) {
    ...
    const verifyError = document.getElementById('forgot-verify-error');
    ...
    if (useMySQL) {
        try {
            ...
            console.error(e);
            showToast("Ralat API MySQL. Menggunakan mod luar talian untuk pengesahan.");
            forgotVerifyFallback(ic, name, verifyError);
        }
    } else {
        forgotVerifyFallback(ic, name, verifyError);
    }
...
function forgotVerifyFallback(ic, name, verifyError) {
    const user = pshUsers.find(u => u.ic === ic && u.name.toLowerCase().includes(name.toLowerCase()));
    if (user) {
        showStep2Reset(ic);
    } else {
        verifyError.textContent = "Ralat! Maklumat diri tidak ditemui dalam rekod JSKK.";
        verifyError.style.display = 'block';
    }
}

function showStep2Reset(ic) {
    document.getElementById('forgot-verified-ic').value = ic;
...
async function handleForgotReset(event) {
    event.preventDefault();
    const ic = document.getElementById('forgot-verified-ic').value;
    ...
    if (newPass !== confirmPass) {
        resetError.textContent = "Ralat! Kata laluan baharu tidak sepadan.";
    ...
    if (useMySQL) {
        try {
            const res = await fetch('api.php?action=forgot_reset', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ic, password: newPass })
            });
            if (!res.ok) throw new Error();
            const data = await res.json();
            
            if (data.success) {
                showToast("Kata laluan berjaya dikemaskini di MySQL!");
                closeForgotWizard();
            } else {
            ...
            console.error(e);
            showToast("Ralat menyimpan ke MySQL.");
        }
    } else {
        const user = pshUsers.find(u => u.ic === ic);
        if (user) {
            user.password = newPass;
            localStorage.setItem('psh_sukan_users', JSON.stringify(pshUsers));
            showToast("Kata laluan berjaya dikemaskini dalam LocalStorage!");
            closeForgotWizard();
        } else {
            resetError.textContent = "Ralat! Masalah tidak terjangka berlaku.";
            resetError.style.display = 'block';
        }
    }
}
